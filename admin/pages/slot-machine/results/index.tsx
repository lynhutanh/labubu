...
import {
  Search, Award, Truck, CheckCircle, Clock, Image as ImageIcon,
} from "lucide-react";
import { slotMachineService, SlotMachineResultResponse } from "../../../src/services/slot-machine.service";
...
export default function SlotMachineResultsPage() {
  const router = useRouter();
  const [results, setResults] = useState<SlotMachineResultResponse[]>([]);
  ...
  const loadResults = async () => {
    try {
      setLoading(true);
      const data = await slotMachineService.searchResults({
  ...
  const handleSearch = () => { if (page === 1) loadResults(); else setPage(1); };

  const handleUpdateDelivery = async (id: string, deliveryStatus: string) => {
    try {
      await slotMachineService.updateDeliveryStatus(id, deliveryStatus);
      toast.success("Cập nhật thành công"); loadResults();
  ...
  return (
    <AdminLayout>
      <Head><title>Kết quả Slot Machine - Labubu Admin</title></Head>
      <div className="flex-1 overflow-y-auto">
        <header className="sticky top-0 z-10 backdrop-blur-lg border-b border-purple-500/30" style={{ background: "rgba(0,0,0,0.3)" }}>
          <div className="px-6 py-4 flex items-center justify-between">
            ...
            <a href="/slot-machine" className="flex items-center gap-2 px-4 py-2 bg-white/10 border border-purple-500/30 text-white rounded-lg font-medium hover:bg-white/20 transition-all">
              ← Cấu hình
            </a>
          </div>
        </header>
        ...
        <main className="p-6 space-y-6">
          <div className="flex flex-wrap gap-3">
            <div className="relative flex-1 min-w-[200px]">
              ...
              <input type="text" value={searchTerm} onChange={e => setSearchTerm(e.target.value)} onKeyDown={e => e.key === "Enter" && handleSearch()} placeholder="Tìm tên, SĐT, email..."
            ...
            <button onClick={handleSearch} className="px-4 py-2 bg-gradient-to-r from-purple-500 to-indigo-600 text-white rounded-lg text-sm font-medium hover:opacity-90 transition-all">Tìm kiếm</button>
          </div>
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="text-purple-400 border-b border-purple-500/30 bg-white/5">
                      ...
                      <th className="text-left py-3 px-4">Phần thưởng</th>
                    ...
                    {results.map(result => {
                      ...
                      return (
                        <tr key={result._id} className="border-b border-purple-500/10 text-purple-200 hover:bg-white/5 transition-colors">
                          ...
                          <td className="py-3 px-4 font-mono text-white">{result.reels?.join(" - ")}</td>
                          <td className="py-3 px-4 text-white font-medium">{result.prizeLabel || "-"}</td>
                          <td className="py-3 px-4">
                            {result.prizeImage ? (
                              <img src={result.prizeImage} alt="" className="w-10 h-10 rounded object-cover" />
                            ) : (
                          ...
                          <td className="py-3 px-4"><span className={typeInfo.color}>{typeInfo.label}</span></td>
                          <td className="py-3 px-4">
                            {result.fullName ? (
                            ...
                            {result.type === "prize" && (
                              <div className="flex justify-center gap-1">
                                {result.deliveryStatus !== "shipped" && (
                                  <button onClick={() => handleUpdateDelivery(result._id, "shipped")} className="px-2 py-1 bg-blue-500/20 text-blue-400 border border-blue-500/30 rounded text-xs hover:bg-blue-500/30 transition-all" title="Đánh dấu đã gửi">
                                    <Truck className="w-3 h-3" />
                                  </button>
                                )}
                                {result.deliveryStatus !== "delivered" && (
                                  <button onClick={() => handleUpdateDelivery(result._id, "delivered")} className="px-2 py-1 bg-green-500/20 text-green-400 border border-green-500/30 rounded text-xs hover:bg-green-500/30 transition-all" title="Đánh dấu đã nhận">
          ...
          {totalPages > 1 && (
            <div className="flex items-center justify-center gap-2">
              <button onClick={() => setPage(p => Math.max(1, p - 1))} disabled={page === 1}
                className="px-4 py-2 bg-white/10 border border-purple-500/30 text-white rounded-lg disabled:opacity-40 hover:bg-white/20 transition-all text-sm">Trước</button>
              <span className="text-purple-300 text-sm">Trang {page}/{totalPages} ({total} kết quả)</span>
              <button onClick={() => setPage(p => Math.min(totalPages, p + 1))} disabled={page === totalPages}
                className="px-4 py-2 bg-white/10 border border-purple-500/30 text-white rounded-lg disabled:opacity-40 hover:bg-white/20 transition-all text-sm">Sau</button>
            </div>
          )}
        </main>
      </div>
