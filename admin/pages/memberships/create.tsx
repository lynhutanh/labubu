...
import { ArrowLeft, Trophy, Save, Key, Gift, Tag, Calendar, ChevronDown, ChevronUp, Plus } from "lucide-react";
...
export default function CreateRankPage() {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        if (!form.key.trim()) return toast.error("Vui lòng nhập mã định danh");
        ...
        try {
            ...
            toast.error(error?.response?.data?.message || error?.message || "Xử lý thất bại");
        } finally {
            setLoading(false);
        }
    };
    ...
    return (
        <AdminLayout>
            <Head>
                <title>Tạo Cấp Bậc - Labubu Admin</title>
            </Head>
                ...
                <main className="p-6 max-w-3xl mx-auto">
                    <form onSubmit={handleSubmit} className="space-y-6">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                ...
                                <div>
                                    <label className="block text-sm font-medium text-purple-300 mb-1 flex items-center gap-1">
                                        <Key className="w-3 h-3" /> Mã định danh *
                                    </label>
                                    <input
                                        type="text"
                                        value={form.key}
                                        onChange={(e) => handleRankChange("key", e.target.value.toLowerCase().replace(/\s+/g, '-'))}
                                        placeholder="VD: gold, diamond..."
                            ...
                            {!showVoucherForm ? (
                                ...
                                <div className="space-y-4 animate-in fade-in slide-in-from-top-2">
                                    ...
                                    <p className="text-xs text-pink-300 font-medium bg-pink-500/10 p-2 rounded">
                                        * Voucher này sẽ tự động gắn vào cấp bậc sau khi được tạo.
                                    </p>
                                </div>
                            )}
                        </div>
                        ...
                        <div className="flex gap-3">
                            ...
                            >
                                {loading ? "Đang xử lý..." : <><Save className="w-4 h-4" /> Tạo Cấp Bậc</>}
                            </button>
                        </div>
                    </form>
                </main>
